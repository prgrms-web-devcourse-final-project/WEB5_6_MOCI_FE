...
export default function StepCategory({ value, onSelect, onNext }: StepCategoryProps) {
  const categories = [
    { name: "카카오톡", icon: <KakaoIcon /> },
    { name: "KTX", icon: <KTXLogo /> },
    { name: "유튜브", icon: <YouTubeLogo className="rounded-lg border border-gray-100 object-contain" /> },
    { name: "쿠팡", icon: <CoupangLogo className="bg-white rounded-lg border border-gray-100 object-contain" /> },
  ...
  return (
    <div className="w-full flex-1 overflow-hidden px-[10vw] sm:px-16 flex flex-col justify-center items-center">
      <h2 className="text-2xl sm:text-3xl font-bold mb-15">무엇을 질문하고 싶으신가요?</h2>
